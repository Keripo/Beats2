...
namespace Beats2.Graphic
{
	...
	public class FontInfo
	{
		...
		public List<FontKerning> kernings = new List<FontKerning>();
	};
		...
		public static FontInfo Parse(string path)
		{
			...
			while ((line = reader.ReadLine()) != null) 
			{
				...
				if (tokens[0] == "common")
				{
					...
					if (pages != 1)
					{
						Debug.LogError("Only one page supported in font. Please change the setting and re-export.");
						return null;
					}
				...
				{
					FontKerning thisKerning = new FontKerning();
	...
	{
		public static FontInfo ParseBMFont(string path)
		{
			FontInfo fontInfo = BMFontTextImporter.Parse(path);
		...
		public static bool BuildFont(FontInfo fontInfo, tk2dFontData target, float scale, int charPadX, bool dupeCaps, bool flipTextureY, Texture2D gradientTexture, int gradientCount)
		{
